...
import { useTeam, useTimes } from "../../Api/competition";
...
export default function Competition({ connectWallet }) {
  ...
  const TeamButton = () => {
    return team.data ? (
      <Link className="default-btn" to={"/competition/" + team.data.leader}>
        View your team
      </Link>
    ) : (
      <Link className="default-btn" to={"/competition/register-team"}>
        Register your team
      </Link>
    );
  };
